...
class Scene(object):
    ...
    def init_episode(self, index: int, randomly_place: bool=True,
        ...
        """
        print("RLBENCH BACKEND SCENE INIT() 1 {}, {}, {}".format(index, randomly_place, max_attempts))
        self._variation_index = index

        if not self._has_init_task:
            self.init_task()
        print("RLBENCH BACKEND SCENE INIT() 2")
        # Try a few times to init and place in the workspace
        attempts = 0
        descriptions = None
        while attempts < max_attempts:
            print("RLBENCH BACKEND SCENE INIT() 3")
            descriptions = self._active_task.init_episode(index)
            print("RLBENCH BACKEND SCENE INIT() 4")
            try:
                if (randomly_place and
                        not self._active_task.is_static_workspace()):
                    print("RLBENCH BACKEND SCENE INIT() 5")
                    self._place_task()
                print("RLBENCH BACKEND SCENE INIT() 6")
        ...
        print("RLBENCH BACKEND SCENE INIT() 7")
