...
class SharedWorldModelSampler():
    ...
    def __init__(self,
        ...
        self.env_change_itr = env_change_itr
    ...
    def initialize(
        ...
        """
        B = self.batch_spec.B
        envs = [self.EnvCls(**self.first_env_kwargs) for _ in range(B)]
        self.first_env_check = True
    ...
    def _change_env(self):
        self.collector.envs[0].env.env.env.shutdown()
        if self.first_env_check:
            envs = [self.EnvCls(**self.second_env_kwargs)]
            self.first_env_check = False
        else:
            envs = [self.EnvCls(**self.first_env_kwargs)]
            print(self.first_env_kwargs.get("config").get("action_mode").arm.name)
            self.first_env_check = True
        self.collector.envs = envs
        agent_inputs, traj_infos = self.collector.start_envs(
            self.max_decorrelation_steps)
        self.agent_inputs = agent_inputs
        self.traj_infos = traj_infos
